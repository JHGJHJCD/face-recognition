...
## הקלדה נתקעת כשהמצלמה דולקת (v1.8, 22/9/2026) — נמדד עם `dev/probe_typing.py`
- **הממצא:** בחלון הקלדת שם (מי זה? / אדם חדש / עריכה) ההקלדה קפאה ל-0.3–0.6 שנ' שוב ושוב — **רק כשהמצלמה דולקת**, בכל מסך (גם בדיקת סרטון).
  לא המודלים (בלי זיהוי בכלל — אותו דבר), לא הטשטוש, לא עדיפות חוטים, לא `GPU_QUEUE_PRIORITY`: **עצם הזרמת המצלמה** (DirectShow, גם מתהליך פייתון
  נפרד בלי Qt — `dev/cam_load.py`) מקפיאה את הדפדפן והחוט הראשי של Qt. רזולוציה/פורמט/קצב לא משנים. זה בדרייבר/מערכת ההפעלה.
- **התיקון:** `modal()` ב-`app.js` שולח `camera/pause` — `LiveWorker.paused` משחרר את המצלמה (`cap.release`) בזמן שהחלון פתוח ופותח מחדש בסגירה (~1 שנ').
  בנוסף: בלי צופה (`last_view` ישן מ-2 שנ' — מסך אחר/חלון) איתור כל 6 תמונות במקום 2; `frameLoop`/`pollLoop` לא מציירים/כותבים מאחורי חלון פתוח.
  נמדד: הקיפאון הכי גרוע 560→~200ms (כמו בלי מצלמה), מסך הזיהוי החי 35→212 פריימים בזמן חלון.
- **כלי המדידה:** `dev/probe_typing.py [--camera] [--page X] [--yt קישור] [--noblur] [--prio] [--spy]` — מקליד באמת בחלון ומודד קיפאונות (Qt + דף).
  ⚠️ `QTest.keyClicks` על QWebEngineView **מקריס** (0xC0000409) — משתמשים ב-`postEvent(QKeyEvent)`. "Qt lag total" מנופח ~6ms/tick
  כשרזולוציית הטיימר של Windows 15.6ms (אחרי שנגן יוטיוב נסגר) — להסתכל על `max` ועל `page frames`.
- **סקירת באגים:** פקודת `/בודק-באגים <נקודה>` (‏`.claude/commands/`) — חפירה על נקודה אחת עד תיקון ושחרור.
